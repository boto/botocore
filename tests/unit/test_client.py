...
class TestAutoGeneratedClient(unittest.TestCase):
    ...
    @mock.patch('botocore.client.DEFAULT_NEW_CREDENTIAL_REFRESH', True)
    def test_auth_error_invalidation_event_registered(self):
        event_emitter = self.create_mock_emitter()
        creator = self.create_client_creator(event_emitter=event_emitter)
        creator.create_client(
            'myservice', 'us-west-2', credentials=self.credentials
        ...
        event_emitter.register.assert_any_call(
            'after-call.myservice', mock.ANY
        )

    @mock.patch('botocore.client.DEFAULT_NEW_CREDENTIAL_REFRESH', False)
    def test_auth_error_invalidation_event_not_registered_when_flag_off(
        self,
    ):
        event_emitter = self.create_mock_emitter()
        creator = self.create_client_creator(event_emitter=event_emitter)
        ...
        self.assertNotIn(
            'after-call.myservice',
            [call[0][0] for call in event_emitter.register.call_args_list],
        )

    def test_expired_token_invalidates_credentials(self):
        credentials_obj = mock.Mock()
        handler = client.AuthErrorInvalidationHandler(
            mock.Mock(return_value=credentials_obj)
        )
        handler.invalidate_on_auth_error(
            parsed={'Error': {'Code': 'ExpiredToken'}},
            context={'signing_access_key': 'access-key'},
        )
        credentials_obj._invalidate.assert_called_once_with('access-key')

    def test_invalid_token_invalidates_credentials(self):
        credentials_obj = mock.Mock()
        handler = client.AuthErrorInvalidationHandler(
            mock.Mock(return_value=credentials_obj)
        )
        handler.invalidate_on_auth_error(
            parsed={'Error': {'Code': 'InvalidToken'}},
            context={'signing_access_key': 'access-key'},
        )
        credentials_obj._invalidate.assert_called_once_with('access-key')

    def test_non_auth_error_does_not_invalidate_credentials(self):
        credentials_obj = mock.Mock()
        handler = client.AuthErrorInvalidationHandler(
            mock.Mock(return_value=credentials_obj)
        )
        ...
        credentials_obj._invalidate.assert_not_called()

    def test_error_code_override_triggers_invalidation(self):
        credentials_obj = mock.Mock()
        handler = client.AuthErrorInvalidationHandler(
            mock.Mock(return_value=credentials_obj)
        )
        handler.invalidate_on_auth_error(
            parsed={'Error': {'Code': 'AccessDenied'}},
            context={
                'error_code_override': 'ExpiredToken',
                'signing_access_key': 'access-key',
            },
        )
        credentials_obj._invalidate.assert_called_once_with('access-key')

    def test_missing_credentials_skips_invalidation(self):
        handler = client.AuthErrorInvalidationHandler(
            mock.Mock(return_value=None)
        )
        result = handler.invalidate_on_auth_error(
            parsed={'Error': {'Code': 'ExpiredToken'}},
            context={'signing_access_key': 'access-key'},
        )
        self.assertIsNone(result)

    def test_invalidation_errors_are_logged(self):
        credentials_obj = mock.Mock()
        credentials_obj._invalidate.side_effect = RuntimeError('error')
        handler = client.AuthErrorInvalidationHandler(
            mock.Mock(return_value=credentials_obj)
        )
        with self.assertLogs('botocore.client', level='DEBUG') as log:
            handler.invalidate_on_auth_error(
                parsed={'Error': {'Code': 'ExpiredToken'}},
                context={'signing_access_key': 'access-key'},
            )
