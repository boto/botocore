...
class TestRestXMLBodySerialization(unittest.TestCase):
    def setUp(self):
        self.model = {
    ...
    def test_body_is_bytes(self):
        request_serializer = serialize.create_serializer('rest-xml')
        input_params = {
            'Timestamp': '0'
        }
        data = request_serializer.serialize_to_request(
            input_params, self.service_model.operation_model('TestOperation'))

        self.assertIsInstance(data['body'], bytes)
