...
from botocore.compat import (
    unquote, json, unquote_str, ensure_bytes, get_md5,
...
def decode_console_output(parsed, **kwargs):
    if 'Output' in parsed:
        try:
            ...
            value = base64.b64decode(parsed['Output'].encode("latin-1")).decode(
                'utf-8', 'replace')
            parsed['Output'] = value
...
def convert_body_to_file_like_object(params, **kwargs):
    if 'Body' in params:
        if isinstance(params['Body'], str):
            params['Body'] = io.BytesIO(ensure_bytes(params['Body']))
        elif isinstance(params['Body'], bytes):
            params['Body'] = io.BytesIO(params['Body'])
