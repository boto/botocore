...
class TestAutoGeneratedClient(unittest.TestCase):
    ...
    def test_does_not_override_configured_presigner(self):
        self.endpoint_data['signatureVersions'] = ['s3', 's3v4']
        event_emitter = mock.Mock()
        creator = self.create_client_creator(event_emitter=event_emitter)
        client = creator.create_client(
            's3', 'us-west-2',
            scoped_config={'s3': {'signature_version': 's3v4'}}
        )

        self.assertNotIn(
            mock.call('choose-signer.s3', utils.default_s3_presign_to_sigv2),
            client.meta.events.register.call_args_list
        )

    def test_keeps_sigv4_on_unknown_region(self):
        self.resolver.get_available_endpoints.return_value = []
        event_emitter = mock.Mock()
        creator = self.create_client_creator(event_emitter=event_emitter)
        client = creator.create_client('s3', 'us-west-98')
    ...
    def test_defaults_to_sigv2_if_available(self):
        self.endpoint_data['signatureVersions'] = ['s3', 's3v4']
        event_emitter = mock.Mock()
        creator = self.create_client_creator(event_emitter=event_emitter)
        client = creator.create_client('s3', 'us-west-2')
    ...
    def test_defaults_to_sigv4_if_sigv2_not_available(self):
        self.endpoint_data['signatureVersions'] = ['s3v4']
        event_emitter = mock.Mock()
        creator = self.create_client_creator(event_emitter=event_emitter)
        client = creator.create_client('s3', 'us-west-2')
