...
class TestAutoGeneratedClient(unittest.TestCase):
    ...
    def test_service_creates_retryhandler(self):
        ...
        # The retry handler is only created for Legacy mode in clients.py, so running
        # this test with other retry modes would fail.
        test_config = Config(retries={'mode': 'legacy'})
        creator.create_client(
            'myservice', 'us-west-2', client_config=test_config
        )
    ...
    def test_service_registers_retry_handler(self):
        # Retry handler returned from ``create_retry_handler``
        ...
        # Retry handler is only created for Legacy mode in clients.py, so running
        # this test with other retry modes would fail.
        test_config = Config(retries={'mode': 'legacy'})
        creator.create_client(
            'myservice', 'us-west-2', client_config=test_config
    ...
    def test_service_retry_missing_config(self):
        # No config means we should never see any retry events registered for legacy retry mode.
    ...
    def test_service_default_retry_available_with_missing_config(self):
        # We should see retry events registered for default (standard) retry mode
        # even when the config is missing
        self.loader.load_data.return_value = {}
