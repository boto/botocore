...
class TestConfig(unittest.TestCase):
    ...
    def test_merge_overrides_only_when_non_default_values(self):
        config = client.Config(
    ...
    def test_merge_default_does_not_override_connect_timeout(self):
        config = client.Config(connect_timeout=55)
        other_config = client.Config()
        new_config = config.merge(other_config)
        self.assertEqual(new_config.connect_timeout, 55)

    def test_merge_override_connect_timeout(self):
        config = client.Config(connect_timeout=55)
        other_config = client.Config(connect_timeout=75)
        new_config = config.merge(other_config)
        self.assertEqual(new_config.connect_timeout, 75)

    def test_merge_default_does_not_override_read_timeout(self):
        config = client.Config(read_timeout=55)
        other_config = client.Config()
        new_config = config.merge(other_config)
        self.assertEqual(new_config.read_timeout, 55)

    def test_merge_override_read_timeout(self):
        config = client.Config(read_timeout=55)
        other_config = client.Config(read_timeout=75)
        new_config = config.merge(other_config)
        self.assertEqual(new_config.read_timeout, 75)
