...
class TestAutoGeneratedClient(unittest.TestCase):
    ...
    def test_client_has_exception_on_meta(self):
        creator = self.create_client_creator()
        service_client = creator.create_client(
            'myservice', 'us-west-2', credentials=self.credentials)
        self.assertTrue(issubclass(service_client.meta.exceptions.TestOperationException,
                                   client.ClientError))
...
class TestClientErrors(TestAutoGeneratedClient):
    def assert_error_response(self, error_response, error_class_name):
        self.endpoint.make_request.return_value = (
        ...
        with self.assertRaises(client.ClientError) as e:
            service_client.test_operation(Foo='one', Bar='two')

        self.assertEqual(e.exception.__class__.__name__, error_class_name)
        error_class = getattr(service_client.meta.exceptions, error_class_name)
        self.assertTrue(isinstance(e.exception, error_class))

    def test_client_makes_call_with_error(self):
        error_data = {'Error': {'Code': 'code', 'Message': 'error occurred'}}
        self.assert_error_response(error_data, 'code')

    def test_error_with_no_wire_code(self):
        error_data = {'Error': {'Code': '404', 'Message': 'Not Found'}}
        self.assert_error_response(error_data, 'ClientError')

    def test_error_with_dot_separated_code(self):
        error_data = {'Error': {'Code': 'InvalidAddress.NotFound', 'Message': ''}}
        self.assert_error_response(error_data, 'InvalidAddressNotFound')

    def test_error_with_empty_message(self):
        error_data = {'Error': {'Code': 'TestOperationException', 'Message': ''}}
        self.assert_error_response(error_data, 'TestOperationException')

    def test_error_with_empty_code(self):
        error_data = {'Error': {'Code': '', 'Message': ''}}
        self.assert_error_response(error_data, 'ClientError')

    def test_error_with_missing_code(self):
        error_data = {'Error': {'Message': 'error occurred'}}
        self.assert_error_response(error_data, 'ClientError')
