...
class TestAutoGeneratedClient(unittest.TestCase):
    ...
    def test_client_custom_retry_config_with_max_attempts(self):
        creator = self.create_client_creator()
        config = botocore.config.Config(retry_config={'max_attempts': 10})
        client = creator.create_client('myservice', 'us-west-2', client_config=config)
        handler = client.meta.events._unique_id_handlers['retry-config-myservice']['handler']

        self.assertEqual(creator._retry_config['max_attempts'], 10)
        self.assertEqual(handler._checker._max_attempts, 10)

    def test_client_custom_retry_config_with_delay_base(self):
        creator = self.create_client_creator()
        config = botocore.config.Config(retry_config={'base': 15})
        client = creator.create_client('myservice', 'us-west-2', client_config=config)
        handler = client.meta.events._unique_id_handlers['retry-config-myservice']['handler']

        self.assertEqual(creator._retry_config['base'], 15)
        self.assertEqual(handler._action.keywords['base'], 15)

    def test_client_custom_retry_config_with_delay_growth_factor(self):
        creator = self.create_client_creator()
        config = botocore.config.Config(retry_config={'growth_factor': 60})
        client = creator.create_client('myservice', 'us-west-2', client_config=config)
        handler = client.meta.events._unique_id_handlers['retry-config-myservice']['handler']

        self.assertEqual(creator._retry_config['growth_factor'], 60)
        self.assertEqual(handler._action.keywords['growth_factor'], 60)
