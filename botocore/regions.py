...
class CredentialBuiltinResolver:
    ...
    def resolve(self, param_definitions, builtins):
        ...
        for builtin_name, builtin_key in builtins_to_resolve.items():
            credential_value = getattr(frozen_creds, builtin_name)
            builtin_value = builtins.get(builtin_key) or credential_value
            builtins[builtin_key] = self._validate_builtin(
...
class EndpointRulesetResolver:
    ...
    def _get_provider_params(
        ...
        self._resolve_builtins(customized_builtins)
    ...
    def _resolve_builtins(self, builtins):
        if self._builtin_resolver is None:
            return

        self._builtin_resolver.resolve(self._param_definitions, builtins)
