...
class TestAutoGeneratedClient(unittest.TestCase):
    ...
    def test_events_are_per_client(self):
        ...
        service_client = creator.create_client('myservice', 'us-west-2')
        service_client.meta.events.register('before-call', handler)
        service_client.test_operation(Foo='one', Bar='two')
        self.assertEqual(len(calls), 1)

        # And if we now create another client, it should have its own
        # copy of the event handlers.
        new_calls = []
        second_handler = lambda **kwargs: new_calls.append(kwargs)
        new_client = creator.create_client('myservice', 'us-west-2')
        new_client.meta.events.register('before-call', second_handler)
        new_client.test_operation(Foo='one', Bar='two')
        # We should have still called the first handler which brings our
        # total calls to 2.
        self.assertEqual(len(calls), 2)

        # And we should also have called our new handler.
        self.assertEqual(len(new_calls), 1)

        # But calling the first client will not invoke second_handler.
        service_client.test_operation(Foo='one', Bar='two')
        self.assertEqual(len(new_calls), 1)
