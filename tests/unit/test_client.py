...
class TestAutoGeneratedClient(unittest.TestCase):
    ...
    def test_client_error_message_for_multiple_positional_args(self):
        creator = self.create_client_creator()
        service_client = creator.create_client(
            'myservice', 'us-west-2', credentials=self.credentials)
        with self.assertRaisesRegexp(TypeError,
                                     'More than one positional argument') as e:
            service_client.test_operation('foo', 'bar')

    def test_cant_specify_both_args_and_kwargs(self):
        creator = self.create_client_creator()
        service_client = creator.create_client(
            'myservice', 'us-west-2', credentials=self.credentials)
        with self.assertRaisesRegexp(TypeError,
                                     "Can't provide both") as e:
            service_client.test_operation('foo', bar='no')

    def test_params_can_be_provided_from_args(self):
        creator = self.create_client_creator()
        service_client = creator.create_client(
            'myservice', 'us-west-2', credentials=self.credentials)
        response = service_client.test_operation({"Foo": "one", "Bar": "two"})
