...
class TestAutoGeneratedClient(unittest.TestCase):
    ...
    def test_service_operations_have_retry_event(self):
        # If an operation has a specific retry config, then it should
        # get its own event registered.
        retry_config = {
            'retry': {
                '__default__': {
                    'max_attempts': 5,
        ...
        self.loader.load_data.return_value = retry_config
        ...
        event_emitter.register.assert_any_call(
            'needs-retry.myservice.TestOperation', mock.ANY,
            unique_id='retry-config-myservice-TestOperation')
