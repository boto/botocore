...
def _assert_serializes_api_version(protocol):
    api_version = '2007-07-07'
    service_model = {
        'metadata': {
            'protocol': protocol,
    ...
    service_model = ServiceModel(service_model)
    request_serializer = serialize.create_serializer(protocol)
    test_operation = service_model.operation_model('TestOperation')
    request = request_serializer.serialize_to_request({}, test_operation)
    assert request['headers'].get('x-amz-api-version') == api_version, request


def test_endpoint_operation_serialization():
    for protocol in SERIALIZERS:
        yield _assert_serializes_api_version, protocol
