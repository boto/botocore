        ...
        self.service_description = {
            ...
            'operations': {
                'TestOperation': {
                    ...
                    'errors': [{'shape': 'TestOperationException'}, {'shape': 'ServiceException'}],
            ...
            'awsQueryCompatible': {
                'ServiceError': {'code': 'AWS.Query.ServiceError'},
                'UnmodeledError': {'code': 'AWS.Query.UnmodeledError'},
            },
        }
        self.retry_config = {
            "retry": {
...
class TestClientErrors(TestAutoGeneratedClient):
    ...
    def test_aws_query(self):
        ...
        cases = {
                'modelled mapped exception': { 'code': 'ServiceError', 'expected': 'AWS.Query.ServiceError'},
                'unmodelled mapped exception': { 'code': 'UnmodeledError', 'expected': 'AWS.Query.UnmodeledError'},
                'unmodelled unmapped exception': { 'code': 'UnmodeledUnmappedError', 'expected': 'UnmodeledUnmappedError'},
                }

        for desc, tc in cases.items():
            error_code = ''
            with Stubber(client) as stub:
                stub.add_client_error(
                    'test_operation', tc['code'], 'service error occurred'
                )
                try:
                    client.test_operation(Foo='one', Bar='two')
                except client.exceptions.ClientError as e:
                    error_code = e.response['Error']['Code']
            self.assertEqual(error_code, tc['expected'], 'Test case for %s failed. Expected %s, got %s' % (desc, tc['expected'], error_code))
