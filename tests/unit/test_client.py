...
class TestAutoGeneratedClient(unittest.TestCase):
    ...
    def test_client_region_overrides_region_from_client_config(self):
        ...
        creator = self.create_client_creator()
        service_client = creator.create_client(
            'myservice', region_name, client_config=client_config)
        self.assertEqual(service_client.meta.region_name, region_name)
    ...
    def test_client_uses_resolver_region_if_none_and_endpoint_url(self):
        with mock.patch('botocore.client.RequestSigner') as mock_signer:
            credential_scope_region = 'us-east-1'
            self.resolver.construct_endpoint.return_value = {
                'partition': 'aws',
    ...
    def test_client_can_use_default_region_if_resolver_provides_it(self):
        with mock.patch('botocore.client.RequestSigner') as mock_signer:
            self.resolver.construct_endpoint.return_value = {
                'partition': 'aws',
                'endpointName': 'us-west-2',
    ...
    def test_client_uses_resolved_endpoint_as_region_name_meta(self):
        self.resolver.construct_endpoint.return_value = {
            'partition': 'aws',
            'endpointName': 'foo',
            'hostname': 'endpoint.url'
        ...
        service_client = creator.create_client(
            service_name='myservice', region_name='us-west-2',
            credentials=self.credentials)
        self.assertEquals('foo', service_client.meta.region_name)

    def test_client_requires_a_region_for_non_s3(self):
        self.resolver.construct_endpoint.return_value = {}
        creator = self.create_client_creator()
    ...
    def test_client_uses_us_east_as_default_s3_region(self):
        self.resolver.construct_endpoint.return_value = {
            'partition': 'aws',
            'endpointName': 'us-east-1',
            'hostname': 's3.amazonaws.com'
        }
        creator = self.create_client_creator(endpoint_prefix='s3')
        service_client = creator.create_client(
            's3', None, credentials=self.credentials)
        self.assertEqual(service_client.meta.region_name, 'us-east-1')

    def test_client_prefers_sigv4_over_v2(self):
        self.resolver.construct_endpoint.return_value = {
            'partition': 'aws',
            'endpointName': 'us-west-2',
            'hostname': 'endpoint.url',
        ...
        service_client = self.create_client_creator().create_client(
            'myservice', 'us-west-2', credentials=self.credentials)
        self.assertEqual('v4', service_client.meta.config.signature_version)

    def test_client_prefers_sigvs3v4_over_all_else(self):
        self.resolver.construct_endpoint.return_value = {
            'partition': 'aws',
            'endpointName': 'us-west-2',
            'hostname': 'endpoint.url',
            'signatureVersions': ['v2', 'v4', 's3v4']
        }
        service_client = self.create_client_creator().create_client(
            'myservice', 'us-west-2', credentials=self.credentials)
        self.assertEqual('s3v4', service_client.meta.config.signature_version)
    ...
    def test_client_s3_addressing_style_with_incorrect_style(self):
        with self.assertRaises(InvalidS3AddressingStyleError):
            client.Config(s3={'addressing_style': 'foo'})
...
class TestConfig(unittest.TestCase):
    def test_can_use_args_to_construct(self):
        config = client.Config(*client.Config.OPTION_DEFAULTS.values())
        for option, default_value in client.Config.OPTION_DEFAULTS.items():
    ...
    def test_can_use_kwargs_to_construct(self):
        config = client.Config(**client.Config.OPTION_DEFAULTS)
        for option, default_value in client.Config.OPTION_DEFAULTS.items():
            self.assertTrue(hasattr(config, option))
            self.assertEqual(getattr(config, option), default_value)

    def test_can_use_mix_of_args_and_kwargs(self):
        config = client.Config('us-east-1', read_timeout=50)
    ...
    def test_merge_overrides_values(self):
        config = client.Config(region_name='us-east-1')
        other_config = client.Config(region_name='us-west-2')
        new_config = config.merge(other_config)
        self.assertEqual(new_config.region_name, 'us-west-2')

    def test_merge_overrides_values_even_when_using_default(self):
        config = client.Config(region_name='us-east-1')
        other_config = client.Config(region_name=None)
        new_config = config.merge(other_config)
