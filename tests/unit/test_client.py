...
class TestAutoGeneratedClient(unittest.TestCase):
    ...
    def test_client_uses_parameter_validation_from_client_config(self):
        client_config = client.Config()
    ...
    def test_client_validates_params_by_default(self):
        ...
        with self.assertRaises(ParamValidationError):
            self._do_operation_with_invalid_parameters(service_client)
    ...
    def test_client_doesnt_validate_params_when_validation_disabled(self):
        ...
        client_config.parameter_validation = False
        service_client = creator.create_client(
            'myservice', 'us-west-2', credentials=self.credentials, client_config=client_config)

        try:
            self._do_operation_with_invalid_parameters(service_client)
        except ParamValidationError:
            self.fail("ParamValidationError shouldn't be raised with validation disabled")

    def _do_operation_with_invalid_parameters(self, service_client):
