...
class TestAutoGeneratedClient(unittest.TestCase):
    ...
    def _create_rpc_cbor_hostname_binding_client(self, *args, **kwargs):
        original_protocol = self.service_description['metadata']['protocol']

        self.service_description['metadata']['protocol'] = 'smithy-rpc-v2-cbor'
        self.service_description['metadata']['targetPrefix'] = 'sampleservice'
        ...
        creator = self.create_client_creator()
        client = creator.create_client('myservice', *args, **kwargs)

        self.service_description['metadata']['protocol'] = original_protocol
        return client
    ...
    def test_client_rpc_cbor_operation_hostname_binding(self):
        client = self._create_rpc_cbor_hostname_binding_client('us-west-2')
        client.test_operation(Foo='bound')

        expected_url = 'https://bound.myservice.amazonaws.com/service/sampleservice/operation/TestOperation'
        self.assertTrue(self.endpoint.make_request.called)
        request_dict = self.endpoint.make_request.call_args[0][1]
        self.assertEqual(request_dict['url'], expected_url)
    ...
    def test_client_rpc_cbor_operation_hostname_binding_validation(self):
        client = self._create_rpc_cbor_hostname_binding_client('us-west-2')
        with self.assertRaises(ParamValidationError):
            client.test_operation(Foo='')
